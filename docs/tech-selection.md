...
- 面试点：为什么"刚性"是 Agent 进生产的前提；状态机 vs 自由循环；幂等与恢复。
...
面试点：为什么元数据用 PG 而非全上 NoSQL；读写分离放应用层还是代理层；向量库演进判据（数据量/并发/召回率 vs 运维成本）；图检索解决什么（关系型、多跳问题，纯向量做不到）。
...
- 面试点：为什么需要路由而不是一套检索打天下；RRF 为什么比加权和稳；路由分类为什么用小模型（成本/延迟）。
...
- LLM 网关（model-gateway，自建模块，基于 infra-ai）：
  ...
  - 面试点：网关是 AI 应用的企业级标配（成本与治理的收敛点）；自建 vs 开源网关的取舍。
...
- 面试点：MCP 解决了什么（工具协议标准化）；敏感度分级与审批是 Agent 落地的安全底线。
...
- 面试点：AI 场景缓存命中率直接等于成本；异步任务的可观测闭环（状态机 + 消息 + 指标三件套）。
...
- 面试点：虚拟线程 vs WebFlux；读写分离的事务边界坑（读从库读到旧数据）。
...
- 面试点：解析质量决定 RAG 上限；切块策略必须用评测数据决策。
